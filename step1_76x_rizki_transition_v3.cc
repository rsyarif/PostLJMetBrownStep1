...
*/   
   ...
   std::vector<int> AllLeptonTrigPresel_PtOrdered; //only for electron
   ...
   std::vector<int> TightLeptonTrigPresel_PtOrdered; //only for electron
   ...
   outputTree->Branch("AllLeptonTrigPresel_PtOrdered",&AllLeptonTrigPresel_PtOrdered);
   ...
   outputTree->Branch("TightLeptonTrigPresel_PtOrdered",&TightLeptonTrigPresel_PtOrdered);
...
/* Not needed anymore done in MiniAod?      
...
*/
      ...
      std::vector<int> TightLeptonTrigPresel; // only for electron
      ...
      std::vector<int> LooseNotTightLeptonTrigPresel; // only for electron
      ...
      std::vector<int> AllLeptonTrigPresel; // only for electron
      ...
      for(unsigned int iel = 0; iel < elPt_singleLepCalc->size(); iel++){
      	//std::cout << "elPt_singleLepCalc->size() = " << elPt_singleLepCalc->size() << std::endl;
		...
		if(isTightEl){
		  ...
		  TightLeptonTrigPresel.push_back(0); //initially set to false
		...
		if(!isTightEl){
		  ...
		  LooseNotTightLeptonTrigPresel.push_back(0); //initially set to false
      ...
      for(unsigned int imu = 0; imu < muPt_singleLepCalc->size(); imu++){
      	//std::cout << "muPt_singleLepCalc->size() = " << muPt_singleLepCalc->size() << std::endl;
...
//       if(isMC && tightlepindex<3) continue; //skip if there is less than 3 tight leptons for MC (not creating ddBKg).
      ...
      AllLeptonTrigPresel_PtOrdered.clear();      
      ...
      TightLeptonTrigPresel_PtOrdered.clear();
      for(unsigned int ilep=0; ilep < tightlepptindpair.size(); ilep++){
      	...
      	TightLeptonTrigPresel_PtOrdered.push_back(TightLeptonTrigPresel.at(tightlepptindpair[ilep].second));
		...
		AllLeptonPt_PtOrdered.push_back(TightLeptonPt.at(tightlepptindpair[ilep].second));
      	...
      	AllLeptonTrigPresel_PtOrdered.push_back(TightLeptonTrigPresel.at(tightlepptindpair[ilep].second));				
      }           
      ...
      for(unsigned int ilep=0; ilep < loosenottightlepptindpair.size(); ilep++){
		...
		AllLeptonPt_PtOrdered.push_back(LooseNotTightLeptonPt.at(loosenottightlepptindpair[ilep].second));
      	...
      	AllLeptonIdx_PtOrdered.push_back(LooseNotTightLeptonIdx.at(loosenottightlepptindpair[ilep].second));
      	AllLeptonTrigPresel_PtOrdered.push_back(LooseNotTightLeptonTrigPresel.at(loosenottightlepptindpair[ilep].second));				
      }           
...
//       	std::cout << "lep " << ilep+1 << " , flavor :" <<  AllLeptonFlavor_PtOrdered[ilep] << " , pt : " <<AllLeptonPt_PtOrdered.at(ilep) << " ,  eta : "<<AllLeptonEta_PtOrdered.at(ilep) << " , phi : "<<AllLeptonPhi_PtOrdered.at(ilep) << " , mass : " << lepM << std::endl;
      ...
      for (unsigned int ilep=0; ilep < AllLeptonPt_PtOrdered.size() ; ilep++){      
		std::cout << "LepFlavor = " << AllLeptonFlavor_PtOrdered.at(ilep) << ", AllLeptonTrigPresel_PtOrdered.at("<<ilep<<") = " << AllLeptonTrigPresel_PtOrdered.at(ilep);
      	if(AllLeptonFlavor_PtOrdered[ilep]==0){
			if(elEta_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep]) < 1.442){
			  if(elSihih_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep]) < 0.012 &&
				 elHoE_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep]) < 0.09 &&
				 elEcalPFClusterIso_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])/AllLeptonPt_PtOrdered[ilep] < 0.37 &&
				 elHcalPFClusterIso_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])/AllLeptonPt_PtOrdered[ilep] < 0.25 &&
				 elDR03TkSumPt_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])/AllLeptonPt_PtOrdered[ilep] < 0.18 &&
				 fabs(elDEtaSCTkAtVtx_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])) < 0.0095 &&
				 fabs(elDPhiSCTkAtVtx_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])) < 0.065
				 ) AllLeptonTrigPresel_PtOrdered.at(ilep) = 1;
			}else{
			  if(elSihih_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep]) < 0.033 &&
				 elHoE_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep]) < 0.09 &&
				 elEcalPFClusterIso_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])/AllLeptonPt_PtOrdered[ilep] < 0.45 &&
				 elHcalPFClusterIso_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])/AllLeptonPt_PtOrdered[ilep] < 0.28 &&
				 elDR03TkSumPt_singleLepCalc->at(AllLeptonIdx_PtOrdered[ilep])/AllLeptonPt_PtOrdered[ilep] < 0.18
				 ) AllLeptonTrigPresel_PtOrdered.at(ilep) = 1;
			}	  			
		}
		std::cout << " ----> " << AllLeptonTrigPresel_PtOrdered.at(ilep)<< std::endl;
      }
...
// 		  std::cout<< "post mindeltar3 = " << mindeltar3 << std::endl;		
      ...
      //8TeV Top Pt Reweighting -- IS THIS NEEDED?
//       float gen_tpt = -999;
